...
export function KnowledgeBaseChatPageDesktop({
  ...
  // Check if user can manage permissions (is creator or has Owner/Maintainer role)
  const canManagePermissions = useMemo(() => {
    if (!knowledgeBase || !user) return false
    // Creator can always manage permissions
    if (knowledgeBase.user_id === user.id) return true
    // Owner or Maintainer can manage permissions
    if (myPermission?.role === 'Owner' || myPermission?.role === 'Maintainer') return true
    return false
